...
export interface DrawerProps {
   ...
   * The AI Draft Response drawer uses this for its gradient header — Doc 04
   * requires the "this is AI-generated" signal to never be scrolled past.
   */
  header?: ReactNode
  children: ReactNode
  footer?: ReactNode
  ...
  return (
    <dialog
      ...
      className={cn(
        ...
        'md:open:animate-[slide-in-right_var(--duration-drawer)_ease-out]',
      )}
    >
      {header}
      <div className="border-border flex items-start justify-between gap-16 border-b p-24">
        <h2 id="drawer-title" className="text-h3 font-display font-semibold">
          {title}
        </h2>
        <button
          type="button"
