...
Phases 0–15 done (scaffolding, schema/migrations, API scaffolding, auth/RBAC, core ticket domain, AI pipeline, SLA + automation engines, notifications/webhooks, search + reporting, frontend foundation, Customer Portal, Agent Console, Admin Dashboard, multi-channel intake, knowledge base, testing & QA). All three client surfaces are built and all three intake channels (portal, email, chat) are live. Next is Phase 16 (Deployment & Delivery) in [06 AgentDesk Implementation Plan.md](docs/06%20AgentDesk%20Implementation%20Plan.md).
...
Phase 15 was QA, and its results live in **[08 Phase 15 QA Report](docs/08%20AgentDesk%20Phase%2015%20QA%20Report.md)** — read it before claiming anything is verified, because it is equally explicit about what is *not*. Three new backend test files (`tests/integration/test_acceptance_criteria.py`, `tests/edge_cases/test_service_degradation.py`, `tests/security/test_secret_exposure.py`) plus `frontend/scripts/a11y.selfcheck.ts`, now part of `npm run selfcheck`.

- **The suite must never reach a real external service.** `.env` holds live Gemini, SMTP *and* IMAP credentials, and `TestClient(app)` runs the lifespan, so the IMAP poller starts too. `conftest.py`'s session `client` fixture blanks `gemini_api_key`, `smtp_host` and `imap_host` — the last two were added in Phase 15 after the suite was found sending real mail from the developer's Gmail at ~4s a call, exhausting its daily quota and tripping the 60s hang watchdog. Any new integration that reads a credential from settings needs the same line. Use the `outbox` fixture to assert on delivery.
- **The test database never rolls back and never resets.** Isolation is "generate unique data" (see `tests/conftest.py`), so rows pile up across runs — that is most of the suite's ~17-minute runtime. `dropdb agentdesk_test` gives a clean baseline. Uniqueness has to hold in the dimension the query under test *sorts* on, not just in the name: the Phase 14 KB retrieval test was unique by title but not by embedding, so every prior run's article tied with it at cosine distance 0.0 and eventually pushed it out of retrieval's `LIMIT 3`.
- **Accessibility is computed, not asserted by comment.** `a11y.selfcheck.ts` parses the tokens out of `index.css` and calculates WCAG contrast. It found three genuine failures on shipped UI, all fixed: white labels on the semantic status colours (`bg-success text-white` was **2.5:1**), `--color-muted` on `--color-sunken` (4.39:1, hitting input placeholders and hovered rows), and `PriorityPill` hardcoding white on an admin-chosen hex. **Any `text-white` on a status colour must use the `-fill` token** (`--color-critical-fill`, `--color-high-fill`, `--color-success-fill`) — the bare token keeps the Doc 07 hue for text, borders, icons and scrubber bars, where nothing sits on top of it. Admin-chosen colours use `readableOn()` from `lib/ui.ts`.
- **Two documented exceptions, deliberate:** AC 2's ">85% classification accuracy" is measured by the product but cannot be gated here (it needs a graded dataset and a checkpoint that is gitignored), and §28's duplicate *detection* is unbuilt — the pipeline retrieves similar tickets only to ground the classifier prompt, and never surfaces a similarity score to the agent.
- **Not executed, and not claimed:** the keyboard-only walkthrough, screen-reader announcement checks, and the cross-browser/responsive pass. All three need a real browser. The global `:focus-visible` floor added in this phase was the missing precondition for the first.
...
| [08 Phase 15 QA Report](docs/08%20AgentDesk%20Phase%2015%20QA%20Report.md) | What is verified, by which test, and what is *not* — read before claiming coverage |
